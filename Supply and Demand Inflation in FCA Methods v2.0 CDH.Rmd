...
family_doctors <- transmute(family_doctors, family_doctors = Sum_count, Hamilton = HAM_CMA, ClinicID = ID)
```
...
#Toy matrix: stepwise
W_s = rbind(c(1, 0.68, 0.22), c(0, 0, 0.68))
...
$$
Local Clinic PPR = L_{j}=\frac{S_j}{\sum_i P_iW_{ij}^{i}}
$$
$$
Local Population Center PPR = A_{i}=\sum_j L_jW_{ij}^{i}
$$
...
```{r binary-impedance-matrices, echo=FALSE}
...
row_sums_binary[row_sums_binary == 0] <- 1 #change to one for those rows where the total is zero to avoid divisions by zero
dis_impedance_binary_row_adjusted <- sweep(impedance_binary, 1, row_sums_binary, FUN = "/") #row standardize

#multiply distance impedance matrix by physicians to get selection matrix and row standardize
selection_binary <- sweep(dis_impedance_binary_row_adjusted, 2, family_doctors$family_doctors, "*")
row_sums_selection_binary <- rowSums(selection_binary)
row_sums_selection_binary[row_sums_selection_binary == 0] <- 1 #change to one for those rows where the total is zero to avoid divisions by zero
impedance_binary_row_adjusted <- sweep(selection_binary, 1, row_sums_selection_binary, FUN = "/") #row standardize

#Column standardize the impedance matrix to ensure that the sum of the level of service preserves the total supply
col_sums_binary <- colSums(selection_binary) #sum weights over rows
col_sums_binary[col_sums_binary == 0] <- 1 #change to one for those rows where the total is zero to avoid divisions by zero
impedance_binary_col_adjusted <- sweep(selection_binary, 2, col_sums_binary, FUN = "/") #column standardize
```

```{r stepwise-impedance-matrices}
...
row_sums_stepwise[row_sums_stepwise == 0] <- 1 #change to one for those rows where the total is zero to avoid divisions by zero
dis_impedance_stepwise_row_adjusted <- sweep(impedance_stepwise, 1, row_sums_stepwise, FUN = "/") #row standardize

#multiply distance impedance matrix by physicians to get selection matrix and row standardize
selection_stepwise <- sweep(dis_impedance_stepwise_row_adjusted, 2, family_doctors$family_doctors, "*")
row_sums_selection_stepwise <- rowSums(selection_stepwise)
row_sums_selection_stepwise[row_sums_selection_stepwise == 0] <- 1 #change to one for those rows where the total is zero to avoid divisions by zero
impedance_stepwise_row_adjusted <- sweep(selection_stepwise, 1, row_sums_selection_stepwise, FUN = "/") #row standardize

#Column standardize the impedance matrix to ensure that the sum of the level of service preserves the total supply
col_sums_stepwise <- colSums(selection_stepwise) #sum weights over rows
col_sums_stepwise[col_sums_stepwise == 0] <- 1 #change to one for those rows where the total is zero to avoid divisions by zero
impedance_stepwise_col_adjusted <- sweep(selection_stepwise, 2, col_sums_stepwise, FUN = "/") #column standardize
```
...
```{r local-clinic-level-of-demand-calculation, echo=FALSE}
#Calculate clinic level of demand using binary impedance 
demand_binary <- (t(impedance_binary) %*% (da_population$POP2011/1000))

#Calculate row-adjusted clinic level of demand using binary impedance 
demand_binary_adjusted <- (t(impedance_binary_row_adjusted) %*% (da_population$POP2011/1000))

#Calculate clinic level of demand using stepwise impedance 
demand_stepwise <- (t(impedance_stepwise) %*% (da_population$POP2011/1000))

#Calculate row-adjusted clinic level of demand using stepwise impedance 
...
los_hamilton <- data.frame(id = family_doctors$ClinicID,
                           family_doctors$family_doctors,
                           family_doctors$Hamilton,
                           los_binary,
                           los_binary_adjusted,
                           los_stepwise,
                           los_stepwise_adjusted)
colnames(los_hamilton) <- c("ClinicID", "FamilyDoctors",
                             "Hamilton_CMA", "LOS_Binary", 
                             "LOS_Binary_Adjusted",
                             "LOS_Stepwise", "LOS_Stepwise_Adjusted")
los_hamilton <- subset(los_hamilton, Hamilton_CMA==1)
```
...
colnames(accessibility) <- c("id", "CMA", "Population", 
                             "Binary", "Binary_Adjusted",
                             "Stepwise", "Stepwise_Adjusted")

# Subset so that it is only Hamilton CMAs entering the disparity calculations below
#accessibility <- subset(accessibility,CMA==537)
```
...
accessibility_comparison <- transmute(accessibility, id, 
                                      Binary = ((Binary/Binary_Adjusted)-1),
                                      Stepwise = ((Stepwise/Stepwise_Adjusted)-1))
```
...
accessibility_disparities <- transmute(accessibility, id, 
                                      Binary = Binary_Adjusted - sum(los_hamilton$LOS_Binary_Adjusted)/nrow(los_hamilton),
                                      Stepwise = Stepwise_Adjusted- sum(los_hamilton$LOS_Stepwise_Adjusted)/nrow(los_hamilton))

#accessibility_disparities <- transmute(accessibility, id, 
#                                      Binary = Binary_Adjusted - sum(los_hamilton$FamilyDoctors)/(sum(accessibility$Population)/1000),
#                                      Stepwise = Stepwise_Adjusted- sum(los_hamilton$FamilyDoctors)/(sum(accessibility$Population)/1000))
```
...
ggmap(basemap) + 
  geom_polygon(data = filter(hamilton_da.t_comparison, Type == "Binary"),
               ...
               color = NA) +
  scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0, name = "Binary Accessibility Inflation Factor") +
  scale_x_continuous(limits = c(-80.3, -79.45), expand = c(0, 0)) +
  scale_y_continuous(limits = c(43.03, 43.5), expand = c(0, 0)) + 
  theme(legend.position="bottom")
```
...
ggmap(basemap) + 
  geom_polygon(data = filter(hamilton_da.t_comparison, Type == "Stepwise"),
                              aes(x = long, y = lat, group = group, fill = Deflation_Factor),
               color = NA) +
  scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0, name = "Stepwise Accessibility Inflation Factor") +
  scale_x_continuous(limits = c(-80.3, -79.45), expand = c(0, 0)) +
  scale_y_continuous(limits = c(43.03, 43.5), expand = c(0, 0)) + 
  theme(legend.position="bottom")
```

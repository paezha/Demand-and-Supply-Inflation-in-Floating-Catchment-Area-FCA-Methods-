...
```{r fig1-example-1, echo=FALSE, fig.cap="\\label{fig:fig1-example-1}A single population center that enters the service areas of two clinics (triangles are clinics, dotted lines are segments of catchment areas)", warning=FALSE, message=FALSE, out.width="95%"}

# Create plot for binary impedance
plot_s1 <- ggplot(df[1:3,], aes(x = X, y = Y, shape = Site)) + 
  geom_point() +
  geom_text_repel(aes(label = Label), vjust = 2.0, size = 3) +
...
```{r fig2-example-2, echo=FALSE, fig.cap="\\label{fig:fig2-example-2}Levels of service of three clinics and accessibility of one population center (triangles are clinics, dotted lines are segments of catchment areas)", warning=FALSE, message=FALSE, out.width="95%"}
# Create plot for binary impedance
plot_s1 <- ggplot(df[1:3,], aes(x = X, y = Y, shape = Site)) + 
  geom_point() +
  geom_text_repel(aes(label = LOS1_b), vjust = 2.0, size = 3) +
  scale_shape_manual(values=c(17, 16)) +
  coord_equal() +
  annotate("path",
...
plot_s2 <- ggplot(df[1:3,], aes(x = X, y = Y, shape = Site)) + 
  geom_point() +
  geom_text_repel(aes(label = LOS1_sw), vjust = 2.0, size = 3) +
  scale_shape_manual(values=c(17, 16)) +
  coord_equal() +
  annotate("path",
           x = df$X[1] + 2.1 * cos(seq(0,2*pi,length.out=100)), alpha = 0.3, 
...
```{r fig3-example-3, echo=FALSE, fig.cap="\\label{fig:fig3-example-3}Three population centers and two clinics (triangles are clinics, dotted lines are segments of catchment areas)", warning=FALSE, message=FALSE, out.width="95%"}
...
```{r fig4-example-4, echo=FALSE, fig.cap="\\label{fig:fig4-example-4}Levels of service of two clinics and accessibility of three population centers (triangles are clinics, dotted lines are segments of catchment areas)", warning=FALSE, message=FALSE, out.width="95%"}
# Create plot for binary impedance
plot_s1 <- ggplot(df, aes(x = X, y = Y, shape = Site)) + 
  geom_point() +
  geom_text_repel(aes(label = LOS2_b), vjust = 2.0, size = 3) +
  scale_shape_manual(values=c(17, 16)) +
  coord_equal() +
  annotate("path",
           x = df$X[1] + 2.1 * cos(seq(0,2*pi,length.out=100)), alpha = 0.3, 
...
plot_s2 <- ggplot(df, aes(x = X, y = Y, shape = Site)) + 
  geom_point() +
  geom_text_repel(aes(label = LOS2_s), vjust = 2.0, size = 3) +
  scale_shape_manual(values=c(17, 16)) +
  coord_equal() +
  annotate("path",
           x = df$X[1] + 2.1 * cos(seq(0,2*pi,length.out=100)), alpha = 0.3, 
...
```{r fig5-example-5, echo=FALSE, fig.cap="\\label{fig:fig5-example-5}Levels of service and accessibility according to 3SFCA and M2SFCA methods (triangles are clinics, dotted lines are segments of catchment areas)", warning=FALSE, message=FALSE, out.width="95%"}
# Create plot for binary impedance
plot_s1 <- ggplot(df, aes(x = X, y = Y, shape = Site)) + 
  geom_point() +
  geom_text_repel(aes(label = LOS2_sel), vjust = 2.0, size = 3) +
  scale_shape_manual(values=c(17, 16)) +
  coord_equal() +
  annotate("path",
           x = df$X[1] + 2.1 * cos(seq(0,2*pi,length.out=100)), alpha = 0.3, 
...
plot_s2 <- ggplot(df, aes(x = X, y = Y, shape = Site)) + 
  geom_point() +
  geom_text_repel(aes(label = LOS2_m), vjust = 2.0, size = 3) +
  scale_shape_manual(values=c(17, 16)) +
  coord_equal() +
  annotate("path",
...
```{r fig6-simulation, echo=FALSE, fig.cap="\\label{fig:fig6-simulation}Setup for the simulation exercise", warning=FALSE, message=FALSE, out.width="95%"}
ggplot() +
...
```{r fig8-simulation-step1, echo=FALSE, fig.cap="\\label{fig:fig8-simulation-step1}Catchment areas in step 1, according to binary and stepwise impedance functions (the weights of the stepwise function are 0.945, 0.600, and 0.424)", warning=FALSE, message=FALSE, out.width="95%"}
# Plot stepwise
plot_sw <- ggplot() +
  geom_sf(data = clinic_buffers_1, aes(fill = id), alpha = 0.3) +
  geom_sf(data = clinic_buffers_2, aes(fill = id), alpha = 0.3) +
...
To see how the overlap of catchment areas impacts the calculations in the first step of the algorithm, we define impedance matrices using the same criteria as for the buffers seen in Figs \ref{fig:fig7-simulation-step1} and \ref{fig:fig8-simulation-step1}. These matrices are shown in Table \ref{tab:table-simulation-impedance}.
...
df_d <- apply(data.frame(D_b_j, D_sw_j, D_sel_j, D_m_j), 2, prettyNum, digits = 5) %>%
  ...
  mutate_if(is.numeric, function(x) {
    cell_spec(x, "latex", color = "black", background = spec_color(x, end = 0.8, alpha = 0.3, option = "plasma", direction = -1))
    }) %>%
  t() %>%
...
kable(df_d, 
  ...
  kable_styling(font_size = 7) %>%
  footnote(general = "Darker cell colors in a row indicate a greater allocation of demand. ") %>%
  landscape()
```
...
kable(df_los, 
  ...
  kable_styling(font_size = 7) %>%
  footnote(general = "Darker cell colors in a column indicate a greater allocation of level of service or accessibility. ") %>%
  landscape()
```
...
los <- data.frame(los_binary, los_binary_adjusted, los_stepwise, los_stepwise_adjusted)
```
...
#
accessibility <- data.frame(DA = da_population$DAUID,
                            Binary = accessibility_binary,
                            Binary_Adjusted = accessibility_binary_adjusted,
                            Stepwise = accessibility_stepwise,
                            Stepwise_Adjusted = accessibility_stepwise_adjusted)
```
...
descriptive_stats <- data.frame(Case = c("Binary", 
                            ...
                            RAPPR = c(sum(family_doctors$family_doctors)/sum(demand_binary),
                            ...
                            mean_LOS = apply(los, 2, mean),
                            sum_LOS = apply(los, 2, sum),
                            mean_Accessibility = apply(accessibility[,2:5], 2, mean),
                            sum_Accessibility = apply(accessibility[,2:5], 2, sum)
                            )
...
```{r fig13-map-demand-inflation-stepwise, echo=FALSE, fig.cap="\\label{fig:fig13-map-demand-inflation-stepwise}Demand inflation, stepwise impedance function", warning=FALSE, message=FALSE, out.width="95%"}
#Calculate inflation factor
...
```{r fig14-map-accessibility-binary, echo=FALSE, message=FALSE, fig.cap="\\label{fig:fig14-map-accessibility-binary}Accessibility, binary impedance function", warning=FALSE, out.width="95%"}
...
```{r fig15-map-accessibility-binary-adjusted, echo=FALSE, message=FALSE, fig.cap="\\label{fig:fig15-map-accessibility-binary-adjusted}Accessibility, adjusted binary impedance function", warning=FALSE, out.width="95%"}
...
```{r fig16-map-accessibility-binary-comparison, echo=FALSE, message=FALSE, fig.cap="\\label{fig:fig16-map-accessibility-binary-comparison}Accessibility Inflation factor, binary impedance function", warning=FALSE, out.width="95%"}
...
Figs \ref{fig:fig18-map-accessibility-stepwise} and \ref{fig:fig19-map-accessibility-stepwise-adjusted} present the results for the stepwise E2SFCA with and without the rectification. The results are qualitatively similar to the 2FSCA, with the expected differences (see Fig \ref{fig:fig20-map-accessibility-stepwise-comparison}). The inflation factors are even more substantial, given the larger catchment areas used.

```{r fig18-map-accessibility-stepwise, echo=FALSE, message=FALSE, fig.cap="\\label{fig:fig18-map-accessibility-stepwise}Accessibility, stepwise impedance function", warning=FALSE, out.width="95%"}
...
```{r fig19-map-accessibility-stepwise-adjusted, echo=FALSE, message=FALSE, fig.cap="\\label{fig:fig19-map-accessibility-stepwise-adjusted}Accessibility, adjusted stepwise impedance function", warning=FALSE, out.width="95%"}
...
```{r fig20-map-accessibility-stepwise-comparison, echo=FALSE, message=FALSE, fig.cap="\\label{fig:fig20-map-accessibility-stepwise-comparison}Accessibility inflation factor, stepwise impedance function", warning=FALSE}
...
Disparity maps for the adjusted binary and stepwise impedance functions are shown in Figs \ref{fig:fig21-map-disparities-binary} and \ref{fig:fig22-map-disparities-stepwise}. These figures reveal the spatial distribution in disparity, with levels of access that are lower than the mean in more rural parts of the city (where travel times are longer and the distribution of physicians is more spatially disperse) compared to levels of access that are greater than the mean in the higher-density and more connected urban center.

```{r fig21-map-disparities-binary, echo=FALSE, message=FALSE, fig.cap="\\label{fig:fig21-map-disparities-binary}Accessibility disparities, adjusted binary impedance function", warning=FALSE, out.width="95%"}
...
```{r fig22-map-disparities-stepwise, echo=FALSE, message=FALSE, fig.cap="\\label{fig:fig22-map-disparities-stepwise}Accessibility disparities, adjusted stepwise impedance function", warning=FALSE, out.width="95%"}
...
# Replicable Research
  ...
  This research was conducted and the paper prepared using `R` and RStudio, along with the following packages: `tidyverse` [@Wickham2017tidy], `knitr` [@Xie2014knitr], `rgdal` [@Bivand2019rgdal], `sf` [@Pebesma2018sf], `gridExtra` [@Auguie2017grid], `raster` [@Hijmans2019raster], `readr` [@Wickham2018readr], `kableExtra` [@Zhu2019kable], `ggthemes` [@Arnold2019ggthemes], `ggrepel` [@Slowikowski2019ggrepel]. Data and source code used in this research are available at https://github.com/paezha/Demand-and-Supply-Inflation-in-Floating-Catchment-Area-FCA-Methods-.
